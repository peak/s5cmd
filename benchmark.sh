...
readonly LARGE_FILE_SIZE=10M
readonly SMALLER_TO_LARGE=10
...
main() {
   ...
   echo -e "${GREEN}Benchmark completed.${NOCOLOR}"
}
...
upload() {
   ...
   hyperfine $HYPERFINE_FLAGS --warmup $WARMUP_COUNT --runs $RUN_COUNT -n $type_old:$OLD "$first_up" -n $type_new:$NEW "$second_up"
}
...
download() {
   ...
   hyperfine $HYPERFINE_FLAGS --warmup $WARMUP_COUNT --runs $RUN_COUNT -n $type_old:$OLD "$first_dl" -n $type_new:$NEW "$second_dl"
}
...
remove() {
   ...
   hyperfine $HYPERFINE_FLAGS --warmup 0 --runs 1 -n $type_old:$OLD "$first_rm" -n $type_new:$NEW "$second_rm"
}
