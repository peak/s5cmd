...
// Special characters that S3 support in object keys:
//   "$" = 24
//   "&" = 26
//   "@" = 40
//   "=" = 3D
...
//   "," = 2C
//   "?" = 3F
//   ASCII character ranges 00–1F hex and 7F: Not tested
func TestRemoteURL(t *testing.T) {
	tests := []struct {
		...
		{
			name: "normal_key_with_prefix",
			input: &URL{
				Scheme:    "s3",
				Bucket:    "bucket",
				Path:      "sub/this-is-normal.txt",
				Prefix:    "sub",
				Delimiter: "/",
			},
			want:    "s3://bucket/sub/this-is-normal.txt",
			wantErr: false,
		},
		{
			name: "key_with_special_characters_1",
			input: &URL{
				Scheme:    "s3",
				Bucket:    "bucket",
				Path:      "sub$1/sub&2/sub@3/sub=4/this-is-special-file+1.txt",
				Prefix:    "sub$1/sub&2/sub@3/sub=4",
				Delimiter: "/",
			},
			want:    "s3://bucket/sub%241/sub%262/sub%403/sub%3D4/this-is-special-file%2B1.txt",
			wantErr: false,
		},
		{
			name: "key_with_special_characters_2",
			input: &URL{
				Scheme:    "s3",
				Bucket:    "bucket",
				Path:      "sub;1/sub:2/sub 3/sub,4/this-is-special-file?2.txt",
				Prefix:    "sub;1/sub:2/sub 3/sub,4",
				Delimiter: "/",
			},
			want:    "s3://bucket/sub%3B1/sub%3A2/sub+3/sub%2C4/this-is-special-file%3F2.txt",
			wantErr: false,
		},
	}
	for _, tc := range tests {
